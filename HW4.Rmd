...
Glass_num = transform(Glass, RI = as.numeric(RI), 
                      Na = as.numeric(Na), Mg = as.numeric(Mg), 
                      Al = as.numeric(Al), Si = as.numeric(Si), 
                      K = as.numeric(K), Ca = as.numeric(Ca), 
                      Ba = as.numeric(Ba), Fe = as.numeric(Fe), 
                      Type = as.numeric(Type))
Glass_num[is.na(Glass_num)]
```

```{r}
correlation_matrix = cor(Glass_num[,c(1:9)])
corrplot(correlation_matrix, order = "hclust")
```

```{r}
highly_correlated <- colnames(Glass_num[, -1])[findCorrelation(correlation_matrix, cutoff = 0.3, verbose = TRUE)]
highly_correlated
```
...
```{r}
control = rfeControl(functions = caretFuncs, number = 2)
results = rfe(Glass_num[,1:9], Glass_num[,10], sizes = c(2,5,9), rfeControl = control, method = "svmRadial")
results
results$variables
# "Ba" "Na" "Al" "Mg" are in two results.
# The two feature selection methods give similar results, but the results are in different orders.
```
...
fullmodel = lm(Glass_num[,1:9],Glass_num[,10],data=Glass_num)
results = step(fullmodel, direction = "backward", trace=FALSE) 
results
```
